...
export default function ItemList({ items, onFormOpen, onItemSelect, onDeleteItem }) {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {sortedItems.map((item) => (
          <div key={item.id} className="flex justify-center items-center  p-4">
            ...
            />
            <button className="text-red-600 hover:underline cursor-pointer" onClick={() => onDeleteItem(item.id)}>
              Delete
            </button>
          </div>
        ))}
